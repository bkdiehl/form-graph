...
export const pizzaForm = defineForm()({
  ...
  resolve: (f: Fields) => {
    ...
    const finish = (allToppings: string[], used: number, budget: number) => {
      const calories = config.baseCalories + Math.round(caloriesOf(allToppings) * (halfAndHalf ? 0.5 : 1));
      const bakeMinutes = 9 + (crust === 'stuffed' ? 3 : 0) + Math.ceil(used / 2);
      const price = config.basePrice + used * 1.25 + (crust === 'stuffed' ? 2 : 0);
      return {
    ...
    if (halfAndHalf) {
      const sideBudget = Math.max(2, Math.floor(config.budget / 2));
    ...
    const toppings = toppingField(f, 'toppings', config.budget);
    return {
      ...base,
      halfAndHalf,
      toppings,
      ...finish(toppings, weightOf(toppings), config.budget),
    };
  },
});
