...
const graph = defineGraph<VmExt>()
  ...
  .field('instanceType', (ctx, ext) => {
    const forPreset = INSTANCE_TYPES.filter((t) => t.preset === ctx.preset);
    ...
    const offered = forPreset.filter(
      (t) =>
        (ext.tier === 'pro' || t.id !== 'c2.metal') && (ext.gpuAvailable || t.preset !== 'gpu')
    );
    const available = offered.filter((t) => t.regions.includes(ctx.region));
    ...
    return {
      ...
      correct: (value: string) => {
        ...
        const inRegion = forPreset.some((t) => t.id === value && t.regions.includes(ctx.region));
        return {
          value: fallback,
          reason: inRegion
            ? ext.gpuAvailable
              ...
              : 'gpu_capacity'
            : 'region_unavailable',
          detail: inRegion ? { tier: ext.tier } : { region: ctx.region },
        };
      },
    };
  })
  .field('vcpus', (ctx, ext) => {
    ...
    const maxVcpus = ext.tier === 'pro' ? 64 : 16;
    return {
      ...slider({ min: 2, max: 64, step: 2, default: 8 }),
      scope: ctx.preset,
      meta: { min: 2, max: maxVcpus, step: 2 },
      correct: (value: number) =>
        value > maxVcpus ? { value: maxVcpus, reason: 'tier_limit', detail: { tier: ext.tier } } : undefined,
    };
  })
  .field('ramGb', (ctx) => {
    ...
    const maxRam = Math.min(256, ctx.vcpus * 4);
    return {
      ...
      correct: (value: number) =>
        value > maxRam
          ? { value: maxRam, reason: 'ram_ceiling', detail: { maxRam, vcpus: ctx.vcpus } }
          : undefined,
    };
  })
  .field('gpuCount', (ctx) => (ctx.preset === 'gpu' ? slider({ min: 1, max: 8, default: 1 }) : null))
  .field('os', enumOf({
    options: [
  ...
  .field('sla', (ctx, ext) =>
    ...
    ctx.spot
      ...
      : enumOf({
          ...
          default: '99.9',
          gate: { '99.99': ext.tier !== 'pro' && 'tier_limit' },
        })
  )
  .field('backups', boolOf())
  .field('backupFrequency', (ctx) =>
    ...
    ctx.backups
      ? enumOf({
          ...
          default: 'daily',
          gate: { hourly: ctx.spot === true && 'spot_hourly_pointless' },
        })
      : null
  )
  .computed('maxRam', (ctx) => Math.min(256, ctx.vcpus * 4))
  .computed('hourly', (ctx) => {
    const typeConfig = INSTANCE_TYPES.find((t) => t.id === ctx.instanceType);
    const backupHourly = ctx.backups
      ? (ctx.ramGb * 0.0002 + ctx.vcpus * 0.001) *
        (ctx.backupFrequency === 'hourly' ? 3 : ctx.backupFrequency === 'weekly' ? 0.4 : 1)
      : 0;
    const hourlyRaw =
      ...
      ctx.ramGb * 0.005 +
      (ctx.gpuCount ?? 0) * 0.9 +
      (ctx.os === 'windows' && ctx.windowsLicense === 'included' ? ctx.vcpus * 0.01 : 0);
    return Math.round((hourlyRaw + backupHourly) * (ctx.spot ? 0.35 : 1) * 1000) / 1000;
  })
  .computed('monthly', (ctx) => Math.round(ctx.hourly * 730 * 100) / 100)
  .computed('bandwidth', (ctx) =>
